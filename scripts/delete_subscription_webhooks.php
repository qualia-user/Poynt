...
if (!is_string($orgId) || $orgId === '') {
    fwrite(STDERR, "ConfigApp::$orgId must be configured to delete subscription webhooks.\n");
...
try {
    $deleted = $webhookService->deleteAllByBusinessId($orgId);

    if ($deleted) {
        fwrite(STDOUT, "Subscription webhooks deleted.\n");
    } else {
        fwrite(STDERR, sprintf("Failed to delete subscription webhooks for org %s.\n", $orgId));

        exit(1);
    }
} catch (Throwable $e) {
    $log->error(
        sprintf('Failed to delete subscription webhooks for org %s: %s', $orgId, $e->getMessage()),
        ['exception' => $e]
    );
